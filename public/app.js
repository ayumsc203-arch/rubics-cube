// ── STATE STORE (LOCAL STORAGE BACKED & SERVER SYNCED) ──
const Store = {
  db: {
    username: "Ayu Solver",
    avatar: "👦",
    ...
    favorites: [],
    completedLessons: ["Beginner 01"],
    settings: { sound: true, cursor: true, theme: "glass" }
  },
  token: null,

  async load() {
    const savedSettings = localStorage.getItem("ayu_cube_settings");
    if (savedSettings) this.db.settings = JSON.parse(savedSettings);

    this.token = localStorage.getItem("ayu_cube_token");
    if (!this.token) {
    ...
    try {
      ...
      if (profRes.ok) {
        ...
        this.db.username = prof.username;
        this.db.avatar = prof.avatar;
        this.db.xp = prof.xp;
        this.db.lvl = prof.lvl;
        this.db.streak = prof.streak;
        this.db.coins = prof.coins;
      } else if (profRes.status === 403 || profRes.status === 401) {
        this.logout();
        return;
      }

      const solvesRes = await fetch('/api/solves', { headers });
      if (solvesRes.ok) {
        this.db.solves = await solvesRes.json();
      }

      const favRes = await fetch('/api/favorites', { headers });
      if (favRes.ok) {
        this.db.favorites = await favRes.json();
      }
    } catch (e) {
      console.warn("Server offline. Using local backup:", e);
      const saved = localStorage.getItem("ayu_cube_store");
      if (saved) this.db = { ...this.db, ...JSON.parse(saved) };
    }
  },
  ...
  async save() {
    ...
    localStorage.setItem("ayu_cube_settings", JSON.stringify(this.db.settings));
  },

  async addSolve(time, scramble, type="3x3 Classic") {
    if (this.token) {
      try {
        const headers = { 'Authorization': `Bearer ${this.token}`, 'Content-Type': 'application/json' };
        const res = await fetch('/api/solves', {
          method: 'POST',
          headers,
          body: JSON.stringify({ time, scramble, type })
        });
        if (res.ok) {
          const result = await res.json();
          this.db.lvl = result.lvl;
          if (result.leveledUp) {
            showToast(`🎉 Level Up! You are now Level ${result.lvl}!`, "🏆");
          }
          await this.load();
          return result.solve;
        }
      } catch (e) {
        console.warn("Failed to record solve on server, using local store:", e);
      }
    }
  ...
  logout() {
    this.token = null;
    localStorage.removeItem("ayu_cube_token");
...
async function validateSolverCube() {
  const msg = document.getElementById("solver-validation-msg");
  if (!msg) return;
  msg.style.color = "var(--text-secondary)";
  msg.textContent = "AI Solver searching... 🔍";

  try {
    const res = await fetch('/api/solve-cube', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ state: solverCube.state })
    });
    
    if (res.ok) {
      const data = await res.json();
      msg.style.color = "var(--accent-green)";
      msg.textContent = `Optimal Solution found in ${data.timeMs}ms!`;
      solverSolutionMoves = data.moves;
      solverSolutionIndex = 0;
      updateSolverPlayerUI();
      showToast("Solution generated by optimal server", "🤖");
    } else {
      const err = await res.json();
      msg.style.color = "var(--accent-red)";
      msg.textContent = `Error: ${err.error}`;
      showToast(err.error || "Validation failed", "⚠️");
    }
  } catch (e) {
    console.warn("Offline fallback solver:", e);
    ...
    if (valid) {
      msg.style.color = "var(--accent-green)";
      msg.textContent = "Solvable state validated (Local Fallback)!";
...
async function loadCommunity() {
  ...
  root.innerHTML = `
    ...
    <div class="grid-cols-2" style="grid-template-columns:1.25fr 0.75fr">
      ...
      <div class="glass-card">
        ...
        <div class="table-wrapper">
          <table class="app-table">
            <thead><tr><th>Rank</th><th>Name</th><th>XP</th></tr></thead>
            <tbody id="leaderboard-body">
              <tr><td colspan="3" style="text-align:center">Loading Ranks...</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      ...
      if (body) {
        body.innerHTML = board.map((row, i) => `
          <tr>
            ...
            <td>${row.username} (Lvl ${row.lvl})</td>
            <td style="color:var(--accent-cyan)">${row.xp} XP</td>
          </tr>
        `).join('');
      }
    }
  ...
  try {
    ...
    if (feedRes.ok) {
      communityPosts = await feedRes.json();
    }
  } catch (e) {
    console.warn("Offline. Using local community feed fallbacks:", e);
    communityPosts = [
      { id: 1, authorName: "MasterCuber", likes: 24, text: "Just discovered a clean PLL insertion for T-Perm!" }
    ];
  }
...
async function createCommunityPost() {
  ...
  if (Store.token) {
    try {
      ...
      console.warn("Failed to save post to server:", e);
    }
  }

  // Offline fallback
  communityPosts.unshift({ id: Date.now(), authorName: Store.db.username, likes: 0, text });
  document.getElementById("comm-post-text").value = "";
  renderCommunityFeed();
  showToast("Post published (offline)!", "🌍");
}
...
async function likePost(id) {
  try {
    ...
    if (res.ok) {
      ...
      const match = communityPosts.find(p => p.id === id);
      if (match) match.likes = updated.likes;
      renderCommunityFeed();
    }
  } catch (e) {
    console.warn("Failed to like post on server:", e);
    const match = communityPosts.find(p => p.id === id);
    if (match) {
      match.likes++;
      renderCommunityFeed();
...
function toggleAuthMode(e) {
  ...
  if (isSignUpMode) {
    ...
    title.textContent = "Login to Ayu Solver";
...
async function handleAuthSubmit(e) {
  ...
  const body = isSignUpMode ? { email, password, username } : { email, password };
  ...
  try {
    ...
    const data = await res.json();
    if (res.ok) {
      localStorage.setItem("ayu_cube_token", data.token);
      showToast(isSignUpMode ? "Registration Successful!" : "Logged in successfully!", "🔑");
      hideAuthModal();
      await Store.load();
      goToPage('dashboard');
    } else {
      showToast(data.error || "Authentication failed", "⚠️");
    }
  } catch (err) {
    console.error("Auth error:", err);
    showToast("Auth Server unreachable", "⚠️");
  }
}
...
function loginAsGuest(e) {
  if (e) e.preventDefault();
  hideAuthModal();
  showToast("Logged in as Guest (Offline)", "👦");
  goToPage('dashboard');
}

// ── FLOATING COACH ASSISTANT DRAWERS ──
...
async function sendCoachQuestion() {
  ...
  const msgBox = document.getElementById("coach-messages");
  const loadingMsg = document.createElement("div");
  loadingMsg.className = "msg ai";
  loadingMsg.textContent = "Ayu AI is typing... 💬";
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      appendCoachMessage(data.answer, "ai");
    } else {
      const data = await res.json();
      appendCoachMessage(data.error || "Coach offline.", "ai");
    }
  } catch (e) {
    loadingMsg.remove();
